...
<head>
    ...
    <script type="text/javascript">
        ...
        function getdata(){
            ...
            $.get("../dbFunction/centrexDetails.php?q=" + reg + "&r=deleteso", {}, function(data, status) {
                console.log('return data ' + data);
                if (data.length !== 0) {
                    if(data[0].SERO_ORDT_TYPE.includes("DELETE")){
                        $("#svtype").val(data[0].SERO_SERT_ABBREVIATION);
                        $("#groupid").val(data[0].GROUP_ID);
                        $("#shortcode").val(data[0].SHORT_CODE);
                    }else{
                        if(data[0].CENTREX != "Y" && data[0].SERO_ORDT_TYPE =="MODIFY-FEATURE"){
                        $("#svtype").val(data[0].SERO_SERT_ABBREVIATION);
                        $("#groupid").val(data[0].GROUP_ID);
                        $("#shortcode").val(data[0].SHORT_CODE);
                        }else{
