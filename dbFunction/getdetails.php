...
oci_free_statement($stid);
	...
	$sql = "SELECT E.EQUP_LOCN_TTNAME, E.EQUP_INDEX, E.EQUP_EQUT_ABBREVIATION, E.EQUP_IPADDRESS, case when CIRT_SERT_ABBREVIATION ='V-VOICE FTTH' THEN SUBSTR(REPLACE(P.PORT_CARD_SLOT,'-',''),2,5)||TO_NUMBER(SUBSTR(P.PORT_NAME,0,2))
ELSE SUBSTR(REPLACE(P.PORT_CARD_SLOT,'-',''),2,5)||SUBSTR(P.PORT_NAME,-2,2) END AS TID
...
(SELECT SOA.SEOA_DEFAULTVALUE  FROM SERVICE_ORDER_ATTRIBUTES SOA   WHERE SOA.SEOA_SERO_ID = SO.SERO_ID AND SOA.SEOA_NAME ='IMSI NO')IMSI_NO ,
(SELECT SOA.SEOA_DEFAULTVALUE  FROM SERVICE_ORDER_ATTRIBUTES SOA   WHERE SOA.SEOA_SERO_ID = SO.SERO_ID AND SOA.SEOA_NAME ='SA_IMS_PACKAGE') SA_IMS_PACKAGE,
(SELECT SOA.SEOA_DEFAULTVALUE  FROM SERVICE_ORDER_ATTRIBUTES SOA   WHERE SOA.SEOA_SERO_ID = SO.SERO_ID AND SOA.SEOA_NAME ='LTE SIGNAL AVAILABILITY') SIG
...
and B.SERO_CIRT_NAME = C.CIRT_NAME
and (B.SERO_ORDT_TYPE like 'MODIFY%' or B.SERO_ORDT_TYPE like 'CREATE%')
...
$s =  $_GET["q"];
	...
	$sql = "SELECT  E.EQUP_LOCN_TTNAME, E.EQUP_INDEX, E.EQUP_EQUT_ABBREVIATION, E.EQUP_IPADDRESS, 
 (SELECT CASE WHEN SOA.SEOA_DEFAULTVALUE  = 'SECONDARY' THEN 2 ELSE 1 END 
FROM SERVICE_ORDER_ATTRIBUTES SOA   WHERE SOA.SEOA_SERO_ID = SO.SERO_ID AND SOA.SEOA_NAME ='VOICE TYPE') 	AS TID
    , (SELECT NETINFO FROM OSSPRG.PSTN_SOURCE_CODE  WHERE PSTN_CODE =SUBSTR(C.CIRT_DISPLAYNAME,2,2)) AS AREA,
...
TRIM(REPLACE(EQUP_LOCN_TTNAME,'-NODE',' '))||'_'||TRIM(REPLACE(REPLACE(EQUP_EQUM_MODEL,'-ISL',' '),'(IPMB)',''))||'_'||SUBSTR(EQUP_INDEX,-2) DNAME
		...
		AND SO.SERO_SERT_ABBREVIATION in ('UPD_CIRCUIT' , 'V-VOICE FTTH')
        AND SO.SERO_CIRT_NAME = C.CIRT_NAME
        AND SO.SERO_STAS_ABBREVIATION NOT LIKE 'C%'";
...
else if($r=='ipendcreateso' ){

$s =  $_GET["q"];
	
	$sql = "select SERO_ID,(SELECT SOA.SEOA_DEFAULTVALUE  
FROM SERVICE_ORDER_ATTRIBUTES SOA   WHERE SOA.SEOA_SERO_ID = SO.SERO_ID AND SOA.SEOA_NAME ='SA_IMS_PACKAGE') SA_IMS_PACKAGE 
        FROM SERVICE_ORDERS SO , CIRCUITS C
        WHERE    C.CIRT_DISPLAYNAME LIKE '$s%'
        AND (SO.SERO_ORDT_TYPE LIKE 'CREATE%' )
		AND SO.SERO_SERT_ABBREVIATION = 'D-IP ENDPOINT'
        AND SO.SERO_CIRT_NAME = C.CIRT_NAME
        AND SO.SERO_STAS_ABBREVIATION NOT LIKE 'C%'
		union
		SELECT CIRT_DISPLAYNAME,(SELECT SOA.SATT_DEFAULTVALUE  
FROM SERVICES_ATTRIBUTES SOA   WHERE SOA.SATT_SERV_ID = C.CIRT_SERV_ID AND SOA.SATT_ATTRIBUTE_NAME ='SA_IMS_PACKAGE') SA_IMS_PACKAGE 
        FROM CIRCUITS C
        WHERE    C.CIRT_DISPLAYNAME LIKE '$s%'
        AND C.CIRT_SERT_ABBREVIATION = 'D-IP ENDPOINT'
        AND C.CIRT_STATUS IN ('INSERVICE','SUSPENDED')";
...
oci_free_statement($stid);
}else if ($r=='ipenddelete' ){
	$sql = "SELECT C.CIRT_SERT_ABBREVIATION, SA1.SATT_DEFAULTVALUE 
FROM CIRCUITS C,SERVICES_ATTRIBUTES SA1
...
while(oci_fetch($stid)){
  $result =  oci_result($stid,oci_field_name($stid, 1))."@".oci_result($stid,oci_field_name($stid, 2))."@1";
}  
oci_free_statement($stid);
}
...
$s =  $_GET["q"];
	...
	$sql = "SELECT *  FROM (SELECT  E.EQUP_LOCN_TTNAME, E.EQUP_INDEX, E.EQUP_EQUT_ABBREVIATION, E.EQUP_IPADDRESS, 
 (SELECT CASE WHEN SOA.SEOA_DEFAULTVALUE  = 'SECONDARY' THEN 2 ELSE 1 END 
FROM SERVICE_ORDER_ATTRIBUTES SOA   WHERE SOA.SEOA_SERO_ID = SO.SERO_ID AND SOA.SEOA_NAME ='VOICE TYPE') 	AS TID
    , (SELECT NETINFO FROM OSSPRG.PSTN_SOURCE_CODE  WHERE PSTN_CODE =SUBSTR(C.CIRT_DISPLAYNAME,2,2)) AS AREA,
(SELECT SOA.SEOA_DEFAULTVALUE OLT_MANUFACTURER 
...
TRIM(REPLACE(EQUP_LOCN_TTNAME,'-NODE',' '))||'_'||TRIM(REPLACE(REPLACE(EQUP_EQUM_MODEL,'-ISL',' '),'(IPMB)',''))||'_'||SUBSTR(EQUP_INDEX,-2) DNAME
        ...
        AND SO.SERO_STAS_ABBREVIATION NOT LIKE 'C%'
		UNION
		SELECT  E.EQUP_LOCN_TTNAME, E.EQUP_INDEX, E.EQUP_EQUT_ABBREVIATION, E.EQUP_IPADDRESS, 
 (SELECT CASE WHEN SOA.SATT_DEFAULTVALUE  = 'SECONDARY' THEN 2 ELSE 1 END 
FROM SERVICES_ATTRIBUTES SOA   WHERE SOA.SATT_SERV_ID = c.CIRT_SERV_ID AND SOA.SATT_ATTRIBUTE_NAME ='VOICE TYPE') 		AS TID
    , (SELECT NETINFO FROM OSSPRG.PSTN_SOURCE_CODE  WHERE PSTN_CODE =SUBSTR(C.CIRT_DISPLAYNAME,2,2)) AS AREA,
...
TRIM(REPLACE(EQUP_LOCN_TTNAME,'-NODE',' '))||'_'||TRIM(REPLACE(REPLACE(EQUP_EQUM_MODEL,'-ISL',' '),'(IPMB)',''))||'_'||SUBSTR(EQUP_INDEX,-2) DNAME
        ...
        AND  C.CIRT_DISPLAYNAME LIKE '$s%'
		AND C.CIRT_STATUS IN ('INSERVICE' ,'SUSPENDED')
		AND C.CIRT_SERT_ABBREVIATION = 'V-VOICE FTTH')WHERE ROWNUM = 1";
...
$sql= "
select CIRT_SERT_ABBREVIATION ,
(select SOA.SATT_DEFAULTVALUE from SERVICES_ATTRIBUTES  soa  where SOA.SATT_SERV_ID = CIRT_SERV_ID and SOA.SATT_ATTRIBUTE_NAME = 'IMSI NO') IMSI_NO,
(select SOA.SATT_DEFAULTVALUE from SERVICES_ATTRIBUTES  soa  where SOA.SATT_SERV_ID = CIRT_SERV_ID and SOA.SATT_ATTRIBUTE_NAME = 'SA_IMS_PACKAGE') MSI,
(select SOA.SATT_DEFAULTVALUE from SERVICES_ATTRIBUTES  soa  where SOA.SATT_SERV_ID = CIRT_SERV_ID and SOA.SATT_ATTRIBUTE_NAME = 'LTE SIGNAL AVAILABILITY') SIGNAL
from  circuits c
where  C.CIRT_DISPLAYNAME like  '".$q."%'
and CIRT_SERT_ABBREVIATION = 'V-VOICE'
AND CIRT_STATUS IN ('INSERVICE' , 'SUSPENDED')
UNION
